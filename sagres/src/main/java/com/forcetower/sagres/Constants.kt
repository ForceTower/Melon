package com.forcetower.sagres

import androidx.annotation.RestrictTo

@RestrictTo(RestrictTo.Scope.LIBRARY_GROUP)
object Constants {
    const val SAGRES_SERVER_PAGE = "http://academico2.uefs.br/Portal"
    const val SAGRES_LOGIN_PAGE = "$SAGRES_SERVER_PAGE/Acesso.aspx"
    val SAGRES_GRADE_PAGE = "$SAGRES_SERVER_PAGE/Modules/Diario/Aluno/Relatorio/Boletim.aspx"
    const val SAGRES_DIARY_PAGE = "$SAGRES_SERVER_PAGE/Modules/Diario/Aluno/Default.aspx"
    val SAGRES_CLASS_PAGE = "$SAGRES_SERVER_PAGE/Modules/Diario/Aluno/Classe/ConsultaAulas.aspx"
    val SAGRES_GRADE_ANY = "$SAGRES_SERVER_PAGE/Modules/Diario/Aluno/Relatorio/Boletim.aspx?op=notas"
    val SAGRES_ENROLL_CERT = "$SAGRES_SERVER_PAGE/Modules/Diario/Aluno/Relatorio/ComprovanteMatricula.aspx"
    val SAGRES_FLOWCHART = "$SAGRES_SERVER_PAGE/Modules/Diario/Aluno/Relatorio/FluxogramaAluno.aspx"
    //Sagres Default Value Constants
    const val LOGIN_VIEW_STATE = ""
    const val LOGIN_VW_STT_GEN = "BB137B96"
    const val LOGIN_VIEW_VALID = "/wEdAATbze7D9s63/L1c2atT93YlM4nqN81slLG8uEFL8sVLUjoauXZ8QTl2nEJmPx53FYhjUq3W1Gjeb7bKHHg4dlob4GWO7EiBlTRJt8Yw8hywpn30EZA="
}
